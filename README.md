...
둘 다 생략하면 오늘 하루를 본다. 한쪽만 줘도 나머지는 오늘 기준으로 채워 보낸다.
서버가 한쪽만 받으면 기간을 걸지 않은 것과 같은 결과를 주기 때문이다.
형식이 어긋난 값은 API 를 부르기 전에 종료 코드 3 으로 거부한다.

`event list` 의 표는 시각과 제목과 캘린더 이름을 낸다.
...
참석자 이름은 `event get` 에만 들어 있다. 목록 응답의 참석자 항목은 비어 있다.

일정 목록은 페이징이 없다. 기간을 좁히는 것이 결과 수를 줄이는 유일한 방법이다.

**`event list` 가 부르는 endpoint 는 Dooray 공식 API 문서에 실려 있지 않다.**
같은 경로로 일정을 만드는 쪽과 일정 하나를 읽는 쪽은 문서에 있는데 목록만 없다.
동작은 실제 호출로 확인했지만 호환을 약속받은 것이 아니라서, 예고 없이 막히거나 응답이 바뀔 수 있다.
멈추면 곤란한 자동화에 넣을 때는 이 점을 감안한다. `calendar list` 와 `event get` 은 문서에 있어 해당하지 않는다.
