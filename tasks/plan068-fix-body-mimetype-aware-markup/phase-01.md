...
**ADR-055 에는 이미 표가 하나 있다.** 형식을 보존하는 경로를 적은 다섯 행짜리 표다.
`post edit` 의 멘션, `post edit --link-task`, `post comment edit` 의 멘션과 링크,
`post comment file upload`, `post comment file delete` 다.
이 phase 가 더하는 실측 표는 그 표와 별개다.
경로가 아니라 표기 종류 넷을 행으로 가진다. 기존 표를 고치지 않고 새 절을 더한다.
...
**사용자에게 물을 수 없는 실행 환경이면 이 항목을 건너뛴다.**
건너뛴 항목은 넷 모두 `확인 못함` 이 되고, phase 02 와 03 의 `text/html` 분기는 거절만 남는다.
건너뛰었다는 것을 ADR 의 실측 절에 적어, 다음에 읽는 사람이 다시 찾을 수 있게 한다.
...
### 4. 실측 절이 다음 phase 의 입력으로 성립하는지 검사한다

phase 02 와 03 은 이 표만 읽고 분기를 만든다. 아래 셋을 하나씩 본다.

- 네 행이 모두 있고, 각 행의 `근거` 열이 비어 있지 않다
- `확인 못함` 이 아닌 행은 `text/html` 표기 열에 실제 문자열이 있다.
  「HTML 앵커」처럼 형태만 적은 행은 구현할 수 없으므로 `확인 못함` 으로 내린다
- 실측에 쓴 프로젝트 코드와 업무 번호와 멤버 ID 가 본문에 남아 있지 않다

셋 중 하나라도 어긋나면 그 자리에서 고친다.
...
표의 네 행이 각각 있는지 본다. 네 낱말을 한 번에 세면 한 행이 네 번 나와도 통과하므로 따로 센다.

```bash
# cwd: <repo root>
for w in "멤버 멘션" "그룹 멘션" "업무 링크" "첨부 reference"; do
  printf '%s: ' "$w"
  grep -c "$w" docs/adr/055-body-mimetype-aware-markup.md
done
```
...
위 명령이 모두 통과하면 확인 결과를 사용자에게 보고한다.
