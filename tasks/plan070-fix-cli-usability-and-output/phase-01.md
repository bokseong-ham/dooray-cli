...
**범위 외**: 알 수 없는 옵션 안내는 phase 02 다. 출력 불일치 셋은 phase 03 이다. 문서는 phase 04 다.
...
4번의 안내는 어느 쪽이 비었는지 밝힌다.

- 첨부도 본문 참조도 없으면 지금 문구 `첨부파일이 없습니다` 를 그대로 쓴다
- `--no-inline` 때문에 제외한 본문 참조가 있으면 그 수를 함께 적는다

```
첨부파일이 없습니다. (본문에 삽입된 파일 2개는 --no-inline 으로 제외했습니다)
```

`--no-inline` 을 준 호출에서는 본문을 받지 않으므로 그 수를 알 수 없다.
이 안내를 내려면 `--no-inline` 이고 첨부가 0건일 때만 본문을 받아야 한다.
**그렇게 하지 않는다.** 그 경우에는 지금 문구를 그대로 내고, 안내는 `--no-inline` 을 쓰는 방법을 적는 데 그친다.
옵션으로 제외하라고 말한 쪽에 조회를 더하는 것은 앞뒤가 맞지 않는다.

`--json` 출력 스키마는 바꾸지 않는다. ADR-031 이 정한 세 키를 그대로 쓴다.

### 4. `src/commands/post/file/download-all.test.ts` 에 확인 다섯을 더한다
