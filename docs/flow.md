...
## 캘린더 조회 흐름 (ADR-063, 문서에 없는 endpoint 정책은 ADR-062)
...
서버가 `timeMin` 하나만 받으면 그것을 무시하고 기간을 걸지 않은 것과 같은 결과를 주므로
**항상 둘 다 보낸다**. 빠진 쪽은 오늘이 아니라 **준 값과 같은 날** 로 채운다.

```
dooray calendar event list --from 2026-10-01   # 10/1 하루 (오늘부터 10/1 까지가 아니다)
dooray calendar event list --to 2026-08-01     # 8/1 하루
```

빠진 쪽을 오늘로 채우면 준 값이 오늘 반대편일 때 `timeMin > timeMax` 가 되어 조용히 0건이 된다.
ISO 값으로 반대쪽을 세울 때는 날짜 부분과 **그 값의 offset** 을 쓴다. 지역 offset 을 붙이면 또 엇갈린다.

`date-range.ts` 가 API 호출 전에 `EXIT_PARAM_ERROR` 로 끊는 경우는 셋이다.
형식이 어긋난 값(서버가 500 을 준다), 실재하지 않는 날짜·시각·offset(`2026-02-31T25:99:99+09:00`),
그리고 사용자가 양쪽을 다 주면서 뒤집어 준 범위다. 뒤집힘 판정만 시각으로 파싱하고 표시용 문자열은 손대지 않는다.

날짜만 준 값을 하루의 시작으로 늘릴 때는 `Date` 가 옮겨 놓은 실제 시각을 되읽는다.
서머타임이 자정에 시작하는 지역에는 그 날 `00:00` 이 없어, 요청한 시각을 그대로 박으면 하루가 한 시간 일찍 시작한다.
