...
- 앵커 표기 셋 중 하나는 의도대로 동작하고 둘은 보이기는 하되 의도와 다르다는 전언을 받았다.
  **그 셋 중 어느 것이 어느 쪽인지는 특정하지 못했다.**
  눌렀을 때 어디로 가는지도 확인받지 못했다
- 댓글 본문의 표기 셋은 확인하지 못했다.
  화면 확인이 업무 본문까지만 이뤄졌다

**그래서 넷을 모두 `확인 못함` 으로 둔다.**
앵커 표기 하나가 동작한다는 전언이 있어도 그것이 멤버 멘션인지 업무 링크인지 첨부인지를 모르면
어느 종류의 HTML 분기를 열지 정할 수 없다. 이 결정은 추측으로 채우지 않기로 했다.
...
개인 프로젝트에 `text/html` 본문의 업무를 만들고 앵커 표기 셋을 한 줄씩 넣는다.
`<a href="dooray://<orgId>/members/<memberId>" title="me">@이름</a>` 형태와
`tasks/<postId>` 형태와 `/files/<fileId>` 형태다.
화면에서 셋을 하나씩 눌러 어디로 가는지 적고, 멘션이면 알림이 오는지도 본다.
