...
댓글 목록 엔드포인트(`GET .../posts/{postId}/logs`)도 `body.mimeType` 을 상세 조회와 동일하게 내려준다.
`post comment edit` 이 상세가 아니라 목록 응답에서 값을 꺼내므로 확인했다.
댓글 402건을 조사해 `text/x-markdown` 400건과 `text/html` 2건 모두 목록과 상세의 값이 같았고, 누락은 0건이었다.
따라서 이 명령을 상세 조회로 바꿀 이유는 없다.
...
- `wiki page edit` 의 비대화형 분기는 보존할 값을 얻으려고 `GET` 을 한 번 더 부른다.
  본문과 `--mime-type` 을 함께 주거나 제목만 수정하면 조회가 없다.
- `--mime-type` 을 단독으로 주면 기존 본문을 그대로 다시 실어 형식만 바꾼다.
  세 명령 모두 비대화형으로 처리하고 `$EDITOR` 를 열지 않는다.
  형식만 바꾸는 엔드포인트가 없어 content 를 함께 보내야 하므로, 위키는 이때 기존 본문을 얻으려 조회한다.
  본문이 없는 위키 페이지는 실을 content 가 없어 `EXIT_PARAM_ERROR` 로 막는다.
- 본문 변경 없이 형식만 달라지면 `warnUnconvertedBody` 가 stderr 로 알린다.
  CLI 는 본문을 변환하지 않으므로 마크다운 본문을 `text/html` 로만 바꾸면 웹에서 원문이 그대로 보인다.
  형식만 되돌리려는 의도일 수도 있어 막지 않고 알리기만 한다.
- 값을 고르는 우선순위(지정값 → 기존값 → markdown 폴백)는 `resolveBodyMimeType` 한 곳에 둔다.
  호출부마다 폴백을 기억하면 빠뜨린다.
  기존 값이 빈 문자열이어도 폴백한다. 그대로 채택하면 `mimeType: ""` 이 요청에 실려 나가기 때문이다.
  요청 타입에서 `mimeType` 은 필수 `string` 이므로 이 폴백이 발동하는 것은 응답이 타입 선언과 어긋났다는 뜻이다.
